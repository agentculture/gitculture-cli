...
description: >
  ...
  Environments. Each gitculture step dry-runs first and waits for confirmation
...
registration — that's a one-time web flow per project, and gitculture by design
...
- After every `gitculture` release, to verify the bootstrap path still works
  end-to-end against a throwaway repo name.

## When **not** to use

- Modifying an existing sibling — use the individual `gitculture repo …` verbs.
...
1. `gitculture repo create --org <org> --description "<…>" <name>` — POST `/orgs/{org}/repos`.
2. `git clone https://github.com/<org>/<name>.git ../<name>` — local clone next to the checkout.
3. `gitculture repo scaffold ../<name>` — shells to `afi cli cite`; writes the
...
4. `gitculture repo env --owner <org> --name pypi --branch main <name>` — PUT pypi env (main only).
5. `gitculture repo env --owner <org> --name testpypi <name>` — PUT testpypi env (any branch).
